...
class AssetsPullCommand extends Command
{
     ...
     * {@inheritdoc}
     */
    protected $name = 'assets:pull';

    /**
     * {@inheritdoc}
     */
    protected $description = 'Pull assets from a remote server';
    ...
    protected function fire()
    {
        ...
        $this->info('Pulling remote assets...');

        foreach ($assetSources as $source) {
            $path = $source->getSourceType()->getSettings()->path;

            $src = $this->parseEnvironmentVariables($path, $remoteConfig);

            $dest = $this->parseEnvironmentVariables($path, $localConfig, true);
    ...
    protected function makeRsyncCommand($src, $dest)
    {
        ...
        if (!empty($this->sshCredentials['host'])) {
            $command->srcHost = $this->sshCredentials['host'];
        }

        if (!empty($this->sshCredentials['user'])) {
            $command->srcUser = $this->sshCredentials['user'];
        }
